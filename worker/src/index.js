...
async function handleRequest(request, env) {
  ...
  if (url.pathname !== '/api/diary') {
    return jsonResponse({ ok: false, error: 'not_found', message: '존재하지 않는 엔드포인트입니다.' }, 404, request, env);
  }

  if (request.method !== 'POST') {
    return jsonResponse({ ok: false, error: 'method_not_allowed', message: '허용되지 않는 메서드입니다.' }, 405, request, env);
  }

  if (!checkOrigin(request, env).ok) {
    return jsonResponse({ ok: false, error: 'origin_not_allowed', message: '허용되지 않은 출처입니다.' }, 403, request, env);
  }

  if (!checkConfig(env).ok) {
    return jsonResponse({ ok: false, error: 'config_missing', message: 'Worker 설정이 누락되었습니다.' }, 500, request, env);
  }

  if (!checkWriteToken(request, env).ok) {
    return jsonResponse({ ok: false, error: 'unauthorized', message: '인증에 실패했습니다.' }, 401, request, env);
  }

  if (!isJsonRequest(request)) {
    return jsonResponse({ ok: false, error: 'invalid_json', message: 'JSON 요청만 처리할 수 있습니다.' }, 400, request, env);
  }
  ...
  try {
    ...
    return jsonResponse({ ok: false, error: 'invalid_json', message: 'JSON 형식이 올바르지 않습니다.' }, 400, request, env);
  }

  const validation = validateDiaryPayload(payload, env);
  if (!validation.ok) {
    return jsonResponse({ ok: false, error: validation.error, message: validation.message }, 400, request, env);
  }

  const created = await createDiaryFileWithRetry(validation.entry, env);
  return jsonResponse(
    {
      ok: true,
      path: created.path,
...
function normalizeTitle(value, date) {
  ...
  return trimmed || formatDateTitle(date);
}

function sanitizeClientId(value) {
  const sanitized = String(value || 'pwa').toLowerCase().replace(/[^a-z0-9_-]/g, '').slice(0, 40);
...
async function createDiaryFileWithRetry(entry, env) {
  ...
  const candidates = [basePath, withRetrySuffix(basePath, 2), withRetrySuffix(basePath, 3)];
...
function makeInboxPath(entry) {
  ...
  return `Diary_formyWife/inbox/new/${entry.date}-${timestamp}-${entry.clientId}.txt`;
}
...
function githubErrorMessage(status) {
  const messages = {
    401: 'GitHub 인증에 실패했습니다. Worker secret 설정을 확인해 주세요.',
    403: 'GitHub 저장 권한이 없거나 요청이 거부되었습니다.',
    404: 'GitHub 저장소 또는 저장 경로를 찾을 수 없습니다.',
    409: '같은 파일명이 이미 존재해서 저장에 실패했습니다.',
